...
fetch("https://inf04.nigdit.men/api/questions").then((res) => res.json()).then( async (data: PytanieT[]) => {    
...
const zaznaczPytania = async (odpowiedzi: PytanieT[], {clickButtons = false, highlightColor = "#008000"}) => {
    ...
    for (let i = 1; i <= pytania.length; i++) {
        ...
        const odpowiedziNaPytanie = [odpowiedzA, odpowiedzB, odpowiedzC, odpowiedzD] as HTMLElement[];
